...
- `Type`s: stored as a H5S_NULL, with the type encoded directly by the `julia_type` attribute. Examples include `Void`, `Any`, and `Int32` (as a type, not a value), and the `julia_type` attribute is, e.g., "Type{Void}".
